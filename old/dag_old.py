...
# rant: inputs is a very tricky thing. its a tensor, and im not sure how to symbolically represent it. i need to represent the following: 0 inputs, 1 input (scalar), m inputs (vector), m^n inputs (tensor). right now, the model is to represent it like this:
# 0 inputs: no .inputs attribute
# 1 input: a single Edge
# m inputs: a list of Edge
# m^n inputs: a nested list of Edge
# this causes two problems: having to check the existence of an attribute, and having to handle a tensor, which python doesnt have native support for (like tensorial indexing)
# this has muddied up my code a lot. we need to fix this
# handling a tensor as a non-list should be perfectly possible. the problem is represent nullary. perhaps store a separate "shape", where dimensionality = len(shape) and if shape is None, tensor is null.
#
# new model:
#
# shape *is* important in knowing the max arity of the callable. so its pretty intuitive to store that. does it deserve its own attribute? this is an implementation detail. so yes, i think its good to have it. lets try it out.
...
def tensor_dimensionality(x):
	'get max nesting depth of iterables in tensor'
	if isinstance(x, (str, bytes)) or not isinstance(x, Iterable):
		return 0
	x = list(x)  # ensure we can iterate multiple times
	if not x:
		return 1
	return 1 + max(tensor_dimensionality(item) for item in x)

from collections.abc import Iterable
def tensor_count_elements(x):
    if isinstance(x, (str, bytes)) or not isinstance(x, Iterable):
        return 1
    return sum(tensor_count_elements(item) for item in x)

from collections.abc import Iterable

def tensor_is_ragged(x):
    if not isinstance(x, Iterable) or isinstance(x, (str, bytes)):
        return False  # scalars aren't ragged

    lengths = []
    for item in x:
        if isinstance(item, Iterable) and not isinstance(item, (str, bytes)):
            lengths.append(len(item))
            if tensor_is_ragged(item):  # recurse down
                return True
...
class FunctionNode(Node):
	...
	def __init__(self, payload:any, metadata:dict=None):
		'since python functions only take a vector shape of function arguments, inputs is either a single Edge or a list of Edge'
		super().__init__(payload, metadata)
		self.outputs:set[Edge] = set()
		
		if callable(payload):
			initial_len = get_arity(payload)[0]
		else:
			initial_len = 0
		
		self.inputs:Edge|list[Edge] = [None] * initial_len	# because dynamically changing list length later is expensive
	...
	def __repr__(self):
		if hasattr(self, 'inputs'):
			inputs_str = f"{tensor_count_elements(self.inputs)} inputs ({tensor_dimensionality(self.inputs)}-D)"
		else:
			inputs_str = 'no inputs'
		return f"<FunctionNode: ID={int_to_str.int_to_str(id(self),int_to_str.HAN)}, {inputs_str}, {len(self.outputs)} outputs, payload={self.payload}>"
	
	def __str__(self):
		output = f"FunctionNode (ID={int_to_str.int_to_str(id(self), int_to_str.HAN)})\n"
		if callable(self.payload):
			min_arity, max_arity, variadic = get_arity(self.payload)
			output += f"payload: {self.payload!r} (min_arity={min_arity}, max_arity={max_arity}, variadic={variadic})\n"
		else:
			output += f"payload: {self.payload!r}\n"
		if hasattr(self, 'inputs'):
			output += f"inputs: {tensor_dimensionality(self.inputs)}-D {'irregular' if tensor_is_ragged(self.inputs) else 'regular'} {type(self.inputs)} of {tensor_count_elements(self.inputs)} elements\n"
			if isinstance(self.inputs, Edge):
				output += f"    {self.inputs!r}\n"
			else:
				for edge in self.inputs:
					output += f"    {edge!r}\n"
		else:
			output += f"inputs: no inputs\n"
		output += f"outputs: {type(self.outputs)} of {len(self.outputs)} elements\n"
		for edge in self.outputs:
			output += f""
		output += f"metadata: {self.metadata!r}\n"
...
class OutputNode(Node):
	...
	def __init__(self, payload:any, metadata:dict=None):
		super().__init__(payload, metadata)
		self.inputs = # a tensor of Edge. theres no way to natively type-hint a tensor in python
...
class Edge:
	...
	"""
	def __init__(self, source:Node, target:Node, index:tuple[int], metadata:dict=None):
	...
	def __repr__(self):
		if hasattr(self.target, 'payload'):
			target_str = str(self.target.payload)
		else:
			target_str = 'no payload'
		return f"<Edge: ID={int_to_str.int_to_str(id(self), int_to_str.GREEK)}, {self.source.payload} → {target_str} @ {self.index}"
...
class Dag:
	...
	def new_inputnode(self, payload, strict:bool=self.strict)->InputNode:
		'create a new InputNode and add it. also return it'
		return self.add_node(InputNode(payload), strict)
	
	def new_functionnode(self, payload, strict:bool=self.strict)->FunctionNode:
		'create a new FunctionNode and add it. also return it'
		return self.add_node(FunctionNode(payload), strict)
	
	def new_outputnode(self, payload, strict:bool=self.strict)->OutputNode:
		'create a new OutputNode and add it. also return it'
		return self.add_node(OutputNode(payload), strict)

	def new_edge(self, source:Node, target:Node, index:None|int|list[int], metadata:dict=None, strict:bool=self.strict)->Edge:
		'create a new edge instance and add it. also return it'
		return self.add_edge(Edge(source, target, index, metadata), strict)
	
	def add_node(self, node:Node, strict:bool=self.strict)->Node:
...
#			raise ValueError(f"{node} already exists")
	...
	def remove_node(self, node:Node, strict:bool=self.strict)->Node:
	...
	def add_edge(self, edge:Edge, strict:bool=self.strict)->Edge:
...
#					raise ValueError(f"{edge.target.inputs[edge.index]} already exists in its target's input")
	...
	def remove_edge(self, edge, strict:bool=self.strict):
		if strict:
			if edge not in self.edges:
				raise ValueError(f"{edge} not found in edges")
			if edge not in edge.source.outputs:
				raise ValueError(f"{edge} not found in source node's outputs")
			if tensor_get_element(edge.target.inputs, edge.index) != edge:
				raise ValueError(f"{edge} not found in target node's inputs")
		self.edges.remove(edge)
		edge.source.outputs.remove(edge)
		if len(edge.index) == 0:
			edge.target.inputs = None
		else:
			tensor_change_element(edge.target.inputs, edge.index, None)
		return edge
	"""
