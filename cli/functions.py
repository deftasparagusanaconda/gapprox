...
def main_menu(ga):
	while True:
		...
		print()
		print("    [0] - advanced options\t\t=", OptionsAdvanced)

		if Options.input_graph_type is None:
			print("\033[91m    [1] - input graph type\t\t=", Options.input_graph_type, '\033[0m')
		else:
			print("    [1] - input graph type\t\t=", Options.input_graph_type)
		
		if Options.function_interpolate is None:
			print("    [2] - interpolation method\t\t=", Options.function_interpolate)
		else:
			print("    [2] - interpolation method\t\t=", Options.function_interpolate.__name__)
		
		if Options.function_approximate is None:
			print("\033[91m    [3] - approximation method\t\t=", Options.function_approximate, '\033[0m')
		else:
			print("    [3] - approximation method\t\t=", Options.function_approximate.__name__)
		
		if Options.function_expression is None:
			print("\033[91m    [4] - expression function\t\t=", Options.function_expression, '\033[0m')
		else:
			print("    [4] - expression function\t\t=", Options.function_expression.__name__)
		
		print("    [5] - error minimization algorithm\t=", Options.function_iter_err_min_alg)

		if Options.function_error is None:
			print("    [6] - error function\t\t=", Options.function_error)
		else:
			print("    [6] - error function\t\t=", Options.function_error.__name__)
		
		if Options.function_stepper is None:
			print("    [7] - stepper function\t\t=", Options.function_stepper)
		else:
			print("    [7] - stepper function\t\t=", Options.function_stepper.__name__)
		
		if Options.output_graph_type is None:
			print("\033[91m    [8] - output graph type\t\t=", Options.output_graph_type, '\033[0m')
		else:
			print("    [8] - output graph type\t\t=", Options.output_graph_type)

		print()
		if input_graph is None:
			print("\033[91m    [a] - input graph\t\t\t=", str(input_graph)[:30], '\033[0m')
		else:
			print("    [a] - input graph\t\t\t=", str(input_graph)[:30])
			
		print("    [b] - output graph\t\t\t=", str(output_graph)[:30])
		print()
		print("[enter] - start")
		print("    [q] - exit")
		print()
...
def get_advanced_options(Options, Components):
	console_clear()
	input("youve opened the advanced options menu")

	return Options

def get_input_graph_type(Options, Components):
	
	input_graph_type = Options.input_graph_type
	InputGraphTypes = Components.InputGraphTypes
...
def _change_input_graph(Options, Components, input_graph):
	input_graph_type = Options.input_graph_type
	InputGraphTypes = Components.InputGraphType
...
def get_input_graph(Options, Components, input_graph):
	input_graph_type = Options.input_graph_type
	InputGraphTypes = Components.InputGraphTypes
