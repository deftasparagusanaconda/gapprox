...
class Quaternion(Hypercomplex):
	...
	def __neg__(self):
		if any(isinstance(value, (Natural, Whole)) for value in (self.value1, self.value2, self.value3, self.value4)):
			raise TypeError("components do not support negation")
		else:
			return Quaternion(-self.value1, -self.value2, -self.value3, -self.value4)
