...
def error_weighted_power_mean(arr_a, arr_b, weight_arr, power = 1):
	if len(arr_a) != len(arr_b) or len(arr_a) != len(weight_arr):
		print("ERROR:\terror_weighted_power_mean(arr_a, arr_b, weight_arr, power) got different lengths!")
		print("\tlen(arr_a)      =", len(arr_a))
		print("\tlen(arr_b)      =", len(arr_b))
		print("\tlen(weight_arr) =", len(weight_arr))
		return -1
	...
	arr_c = [abs(a-b) for a, b in zip(arr_a, arr_b)]
	import scipy.stats
	return scipy.stats.pmean(arr_c, power, weights=weight_arr)

# https://en.wikipedia.org/wiki/Mean_absolute_percentage_error
def error_MAPE(arr_actual, arr_forecast):
	if len(arr_actual) != len(arr_forecast):
		print("ERROR:\terror_MAPE(arr_actual, arr_forecast) got different lengths!")
		print("\tlen(arr_actual)   =", len(arr_actual))
		print("\tlen(arr_forecast) =", len(arr_forecast))
...
def error_SMAPE(arr_actual, arr_forecast):
	if len(arr_actual) != len(arr_forecast):
	print("ERROR:\terror_SMAPE(arr_actual, arr_forecast) got different lengths!")
		print("\tlen(arr_actual)   =", len(arr_actual))
		print("\tlen(arr_forecast) =", len(arr_forecast))
...
print(error_SMAPE(myarr1, myarr2))
"""
