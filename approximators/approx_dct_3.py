...
def approx_dct_3(inarr, return_type = "values"):
	params = dct(inarr, type=3, norm="forward")
	...
	N = len(inarr)
	x = symbols("x")
	terms = [2 * params[ i] * cos(pi*x*(2*i+1)/(2*N)) for i in range(N)] 
	...
	if "values" == return_type:
		...
		raise ValueError("approx_dct_3() got invalid return_type")
	
	raise RuntimeError("approx_dct_3() reached unexpected execution path")
